...
namespace DAL.Test {
    public class TestReservation {
        ...
        public void TestInsertIntoReservationTrue () {
            Reservation reser = new Reservation (1, 1, 1, "A1 A2 A3 A4 A5",12345, 225000.0, DateTime.Now);
            Assert.True (reservation.InsertIntoReservation (reser));
        }

        [Fact]
        public void TestInsertIntoReservationFail () {
            Reservation reser = new Reservation (1, 1, 1, null,12345 ,225000.0, DateTime.Now);
            Assert.False (reservation.InsertIntoReservation (reser));
        }

        [Fact]
        public void TestInsertIntoReservationFail1 () {
            Reservation reser = new Reservation (1, 0, 1, null,123456, 225000.0, DateTime.Now);
            Assert.False (reservation.InsertIntoReservation (reser));
        }

        [Fact]
        public void TestInsertIntoReservationFail2 () {
            Reservation reser = new Reservation (1, 0, 0, null, 12345,225000.0, DateTime.Now);
            Assert.False (reservation.InsertIntoReservation (reser));
        }
        [Fact]
        public void TestInsertIntoReservationFail3()
        {
            Reservation reser = new Reservation (1, 1, 1, "",12345 ,225000.0, DateTime.Now);
            Assert.False (reservation.InsertIntoReservation (reser));
        }
