...
namespace BL.Test {
    public class ReservationBLTest {
        ...
        public void TestInsertIntoReservation () {
            ReservationBL re = new ReservationBL ();
            Reservation reser = new Reservation (2, 1, 2, "A5 A6 A7 A8", 10000.00, DateTime.Now);
        ...
        public void TestInsertIntoReservationFail () {
            ReservationBL re = new ReservationBL ();
            Reservation reser = new Reservation (0, 0, 0, "A5 A6 A7 A8", 10000.00, DateTime.Now);
            Assert.False (re.InsertIntoReservation (reser));
        }
        [Fact]
        public void TestInsertIntoReservationFail1 () {
            ReservationBL re = new ReservationBL ();
            Reservation reser = new Reservation (2, 1, 2, null, 10000.00, DateTime.Now);
            Assert.False (re.InsertIntoReservation (reser));
        }
